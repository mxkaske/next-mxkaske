...
Why can't we use a `getServerSideProps`? Simply because we can't use both. For blog posts, we should stick to static sites as they are perfect for SEO. Instead, Next.js allows us to do Incremental Staic Regeneration which allows us to regenerate the page if it got staled.
...
Each time a new **/writing/blog-post-slug** page is opened, we will make a small API request that will increase our counter by one and automatically revalidate the same page so that, once regenerated (might take a few seconds), we will have the new page view numbers on the static page.
...
We need a little API that we can call from the client and increase the counter number for a blog post `slug`. In there, we will also call the [`res.revalidate`](https://nextjs.org/docs/api-routes/response-helpers) to regenerate a static page with the new counter. Let's see how it looks like:
...
Once implemented, you should see similar data:

![upstash data](/assets/blog/page-views/upstash-data.png)

### Final words

That's it! We have now a view counter implemented with Next.js and Upstash and it will incrementally regenerate our static pages.
